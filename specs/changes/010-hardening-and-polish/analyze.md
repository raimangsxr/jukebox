...
|----|----------|---------|------------|
| A1 | **MEDIUM → dismissed at implementation** | Flagged that FR-019 (apply theme on `/participar`) might lack a data path. | **Dismissed**: reading the code showed `ParticipantStateResponse` **already includes** the full `event_config` (with `theme`), so `/participar` already receives it. No `ParticipantStateResponse` change and no `T057` were needed; FR-019 note corrected. The frontend simply reads `state.event_config.theme`. |
